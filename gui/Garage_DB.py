...
# NOTE : handle what if a person has more than one car ( if u scan car plates its fine but scanning id will not tell which car )
###########################################################################
def connect_db():
    """
    ...
    """
   #  abs_path = os.path.dirname(os.path.abspath(__file__)) # directory path of the app
    db_path = ".\smart_garage.db"
    connect_obj = None
...
# 30 test sample of non real people info ( 1st two records is expired and about to expire license for later use)
    cursor.execute(
      ...
      ('59482143698237', 'Ahmed Khalid', 'Mercedes E-Class 2018',
       '2025-07-31', 'cmgivrat@gmail.com'),
