...
# TODO : tracking
# TODO : enhance to work with real id cards ( mostly will change tesseract psm mode to parse )
# _#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#
...
---
    ...
    # deep copy ->  not share any data with frame (may take more mem. but efficent than COW copy + solved '=' problem)
    ...
    # frame_to_show  has no shape attribute (UMat obj)
    y_center, x_center = [int(x) // 2 for x in frame_shape]
    # top left
    x1, y1 = x_center - id_dimension[0] // 2, y_center - id_dimension[1] // 2
    # bot_right
    x2, y2 = x_center + id_dimension[0] // 2, y_center + id_dimension[1] // 2
    # make rec_in_center
    rec_spec['top_left_coordinate'], rec_spec['bot_right_coordinate'] = (
        x1, y1), (x2, y2)
...
def get_pos(frame_shape: tuple, id_dimension: tuple) -> list:
    ''' return : [(x1 , y1 ) , (x2 , y2)] '''
    y_center, x_center = [int(
        x) // 2 for x in frame_shape]  # frame_to_show  has no shape attribute (UMat obj)
    # top left
    x1, y1 = x_center - id_dimension[0] // 2, y_center - id_dimension[1] // 2
    # bot_right
    x2, y2 = x_center + id_dimension[0] // 2, y_center + id_dimension[1] // 2
...
# default gpu set to nvidia cuda == 1
def check_gpu_accl(_default_gpu: int = 1) -> tuple:
    """
...
---
    ...
    * Returns:
    ...
    if cuda_enabled and cuda_available:
        ...
        if _default_gpu == 1:
            # even if opencl is avilable gpu def is nvidia if == 1
...
# in case gpu acceleration is not enough to maintain stable 30fps
def enable_multithreading(thread_no: int = 4):
...
# NOTE: DETECT keypoints and create the decriptors for ref.img and img_to_cmp
    ...
    # two algorithms in ORB : corner detection and binary descriptor extraction
    detect_obj = cv2.ORB_create()
...
# NOTE: MATCH and save best matches
    ...
    if testing_mode == True:  # TESTING
        print(
            f"ref_desc type : {type(ref_desc)}  img_desc  type {type(img_desc)}")

    matched_descs = match_obj.match(ref_desc, img_desc)
    # match() returns DMatch_Obj = [img_desc_indx , ref_img_desc_indx , distance]

    # each x is a matched_descs obj
    matched_descs = sorted(matched_descs, key=lambda x: x.distance)

    tolerance = 16
    # get only best n matches (smallest distance)
    good_matches = matched_descs[:tolerance]
...
def get_trans_mat(good_matches: cv2.DMatch, key_points: tuple) -> np.ndarray:
    ...
    # 'pt' is the pixel coordinate of a keypoint
    ref_good_pts = np.float32([ref_kp[m.queryIdx].pt for m in good_matches])
    # make 3D mat of coordintes [ [x1,y1] , [x2,y2] ..]
    ref_good_pts = ref_good_pts.reshape(-1, 1, 2)
    ...
    trans_mat, choosed_matches_mask = cv2.findHomography(
        ref_good_pts, img_good_pts, cv2.RANSAC, 5.0)
...
def affine_trans(_skew_angle: int, img_coord: tuple, _image_to_skew: np.ndarray) -> np.ndarray:
    ...
    affine_rot_mat = cv2.getRotationMatrix2D(
        center, _skew_angle, scale=1.0)  # 1.0 is image scale factor
    rotated_img = cv2.warpAffine(_image_to_skew, affine_rot_mat,
...
# lets call it de-rotate for now
def deskew_img(img_to_skew: np.ndarray, ref_img: np.ndarray, **extraArgs) -> np.ndarray:
    """
---
    Returns:
...
"""
    ...
    skip = False
    # if skew angle is big render red rect for now
    skew_angle = extraArgs['skew_angle']
    ...
    if extraArgs['count'] % frames_to_skip_procs == 0:
        if extraArgs['count'] > 2:
            if _is_valid == 1:
                ...
                frame, pos = make_rectangle_obj(
                    _frame, _id_dimension, clr, _frame_shape=extraArgs['frame_shape'])
                frame = make_timer_obj(frame, pos, timer)
                cv2.imshow("Camera", frame)
            elif _is_valid2 == 1:
                frame2, pos2 = make_rectangle_obj(
                    _frame, _id_dimension, clr, _frame_shape=extraArgs['frame_shape'])
    ...
    _frame = cv2.threshold(
        _frame, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]

    # if testing_mode == True :
    # 	cv2.imshow("TESTING : show image threshed" , _frame)#TESTING

    # deep copy _frame ( efficient but takes more mem.)
    no_deskew = cv2.UMat(np.array(_frame.get()))
    ...
    # little bit  better for text(more horizentally focused)
    ...
    if testing_mode == True:  # TESTING
        ...
        temp_no_deskew_shape = no_deskew.get(
        ).shape[0], no_deskew.get().shape[1]
        print(f" not_deskewed shape : {temp_no_deskew_shape}")
        print(f" deskewed shape : {temp_deskew_shape}")
        cv2.imshow("TESTING : show image final_deskewed",
                   img_final_deskew)  # TESTING
        cv2.imshow("TESTING : show image final_not_deskewed",
                   img_final_no_deskew)  # TESTING
...
---
    ...
    # always 'numeric' for type 0 id (default)
    id_values_type: str = id_char_type
...
# _#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#
# NOTE: also use it when no GPU
def read_simple_card_opencl(vid: cv2.VideoCapture, vid_specs: list, id_card_specs: dict, is_valid=False, is_valid2=False) -> str:
    """
    * Args:
            vid
...
---
    * Returns:
    ...
    fps = vid_specs[0]
    # fps * length_of_video = total number of frames
    vid_time_cnt = fps * vid_length_sec
    ...
    while vid_time_cnt > 0:  # start capture camera for vid_length_sec
        ...
        img_final_deskew, img_final_no_deskew, skip, skew_angle = process_vid_frame(
            frame, _id_dimension=id_dimension, _is_valid=is_valid,  _is_valid2=is_valid2, _ref_img=ref_img, frame_shape=frame_shape, count=vid_time_cnt, skew_angle=skew_angle)

        # skip this frame for speed or err handle
        if type(img_final_deskew) == type(False) or type(img_final_no_deskew) == type(False):
        ...
        if testing_mode == True:  # TESTING
            end_time = time.time()
            print(
                f"type img_final_deskew before cvt to mat: {type(img_final_deskew)}")
            print(
                f"type img_final_no_deskew before cvt to mat: {type(img_final_no_deskew)}")
            actual_frametime = end_time - start_time
            actual_fps = 1 / actual_frametime
            max_rec_frametime_min_fps[0] = max(
                max_rec_frametime_min_fps[0], actual_frametime)
            max_rec_frametime_min_fps[1] = min(
                max_rec_frametime_min_fps[1], actual_fps)
            min_rec_frametime_max_fps[0] = min(
                max_rec_frametime_min_fps[0], actual_frametime)
            min_rec_frametime_max_fps[1] = max(
                max_rec_frametime_min_fps[1], actual_fps)
            print(
                f"#### actual Frametime(sec) and FPS: {actual_frametime} , {actual_fps} ####")
            print(
                f"#### TARGET frametime and fps (sec) {frametime / 1000} , {fps} ####")
...
'''
    ...
    while i > 0:
        ...
        cfg = "--psm 11 --oem 3"
        # Sparse text with osd. Find as much text as possible in no particular order.
        cfg2 = "--psm 12 --oem 3"
        ...
        # edits valid_ids_freq inside
        is_valid, *_ = search_id(id_card_specs['id_char'],
                                 scanned_image=imgstr, valid_ids_freq=valid_ids_freq)
        # edits valid_ids_freq inside
        is_valid2, *_ = search_id(id_card_specs['id_char'],
                                  scanned_image=imgstr2, valid_ids_freq=valid_ids_freq2)
        ...
        i -= 1
        n = 1
        # after frames  acccepted acuurate for 1 second
        validate_after = (n * fps) // 2

        # perfect match  if still valid for a whole n (sec)
        if valid_cnt >= validate_after:
            all_success = True
            ...
            # Sucess but not perfect match
            return True, max(valid_ids_freq,  key=valid_ids_freq.get)

        # perfect match  if still valid for a whole n (sec)
        if valid_cnt2 >= validate_after:
            all_success = True
            # get only one id -> the higest freq ( if two has same freq choose the first to be inputed in freq_arr)
            ...
            # Sucess but not perfect match
            return True, max(valid_ids_freq2,  key=valid_ids_freq2.get)
    ...
    # fail but return best guess
    return False,  max(int(max_skewed), int(max_un_skewed))
 # _#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#
...
def save_ref_img_db(name: str = "ref_rot_img"):  # only use manually
    ...
    ref_img = cv2.threshold(
        ref_img, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]
...
def get_ref_img_db(img_name: str = "ref_rot_img") -> np.ndarray:
    ...
    ref_img = cv2.imdecode(np.frombuffer(
        ref_img_encoded, dtype=np.uint8, ), cv2.IMREAD_GRAYSCALE)
...
---
    ...
    ids_specs = [default_id_specs, lisence_specs,
                 car_plate_specs]  # 0 is the default id
    ...
    if active_gpu_api == 1:  # Cuda
        if id_type_indx == 0:  # default id card
            final_status, scanned_id = read_simple_card_opencl(
                vid, vid_specs, ids_specs[0])
    ...
    if testing_mode == True:
        print(
            f" highest recorded frametime  (sec) , lowest  fps  : {max_rec_frametime_min_fps}")
        print(
            f" lowest  recorded frametime  (sec) , highest fps  : {min_rec_frametime_max_fps}")
